// /pages/manualComputationPage.js
(function () {
  ...
  function Computation() {
    ...
    useEffect(() => {
      localStorage.setItem('threadco_price_breaks_v1', JSON.stringify(priceBreaksFrontColor));
    ...
    const frontSurcharge = useMemo(() => colorSurcharge(clampNum(frontColors, { min: 0 })), [frontColors]);
    const backSurcharge  = useMemo(() => colorSurcharge(clampNum(backColors,  { min: 0 })), [backColors]);
    ...
    const unitPrice = useMemo(() => {
      const raw = clampNum(basePrice, { min: 0 }) + frontSurcharge + backSurcharge;
      return Math.max(0, raw - (appliedBreak?.deduct || 0));
    }, [basePrice, frontSurcharge, backSurcharge, appliedBreak]);
    ...
    const resetBreaksFrontColor = () => setPriceBreaks(defaultBreaksFrontColor);
